...
if (!KEY) { console.error("❌ TMDB_API_KEY missing in .env!"); process.exit(1); }

async function safeFetch(url, retries = 3) {
  for (let i = 0; i <= retries; i++) {
    try {
      const res = await axios.get(url, { timeout: 10000, headers: { "Accept-Encoding": "gzip" } });
      return res.data;
    } catch (e) {
      ...
      await new Promise(r => setTimeout(r, isRetry ? 1000*(i+1) : 400*(i+1)));
    }
  }
}

const moodMap = {
  happy:"35,10751", sad:"18", thriller:"53,80", romantic:"10749",
  action:"28", scifi:"878", horror:"27", animation:"16"
};
...
app.get("/api/mood/:type", async (req, res) => {
  ...
  const data = await safeFetch(`${TMDB}/discover/movie?api_key=${KEY}&with_genres=${genres}&sort_by=popularity.desc&page=1`);
...
app.get("/api/search", async (req, res) => {
  ...
  const data = await safeFetch(`${TMDB}/search/movie?api_key=${KEY}&query=${encodeURIComponent(q)}&include_adult=false`);
  res.json(data?.results || []);
});
...
app.get("/api/movie/:id", async (req, res) => {
  ...
  for (const m of [...(recommendations?.results||[]), ...(similar?.results||[])]) {
    if (!seen.has(m.id) && m.poster_path) { seen.add(m.id); recs.push(m); }
  }
  if (recs.length < 8 && credits?.crew) {
    const director = credits.crew.find(c => c.job === "Director");
    if (director) {
      const dirMovies = await safeFetch(`${TMDB}/person/${director.id}/movie_credits?api_key=${KEY}`);
  ...
  res.json({
    ...
    crew:  credits?.crew?.filter(c=>["Director","Screenplay","Story","Writer"].includes(c.job))?.slice(0,4) || [],
...
app.get("*", (_req, res) => res.sendFile(join(__dirname, "public", "login.html")));
